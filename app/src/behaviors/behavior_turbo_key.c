...
static int new_turbo(uint32_t position, const struct behavior_turbo_config *config, struct active_turbo **turbo) {
    for (int i = 0; i < ZMK_BHV_TURBO_MAX_ACTIVE; i++) {
...
#define TURBO_INST(n)                                                 \
    ...
    DEVICE_DT_INST_DEFINE(n, behavior_turbo_key_init, NULL, NULL, &behavior_turbo_config_##n, APPLICATION, CONFIG_KERNEL_INIT_PRIORITY_DEFAULT, &behavior_turbo_key_driver_api);
