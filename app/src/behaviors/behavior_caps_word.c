...
static bool caps_word_is_caps_includelist(const struct behavior_caps_word_config *config,
    ...
    for (int i = 0; i < config->continuations_count; i++) {
        ...
        LOG_DBG("Comparing with 0x%02X - 0x%02X (with implicit mods: 0x%02X)", continuation->page, continuation->id, continuation->implicit_modifiers);
...
static int caps_word_keycode_state_changed_listener(const zmk_event_t *eh) {
    ...
    for (int i = 0; i < DT_NUM_INST_STATUS_OKAY(DT_DRV_COMPAT); i++) {
        ...
        if ((!caps_word_is_alpha(ev->keycode) || !config->ignore_alphas) &&
            ...
            !caps_word_is_caps_includelist(config, ev->usage_page, ev->keycode, ev->implicit_modifiers)) {
...
static int behavior_caps_word_init(const struct device *dev) {
    ...
    #define KP_INST(n)                                                                                \
        ...
        DEVICE_DT_INST_DEFINE(n, behavior_caps_word_init, NULL, &behavior_caps_word_data_##n, &behavior_caps_word_config_##n, APPLICATION, CONFIG_KERNEL_INIT_PRIORITY_DEFAULT, &behavior_caps_word_driver_api);
